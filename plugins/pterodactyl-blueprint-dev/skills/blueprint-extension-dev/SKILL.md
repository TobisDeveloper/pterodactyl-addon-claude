...
- Backend controllers live in the `requests.app` directory with `namespace {appcontext};` — Blueprint rewrites this placeholder at install time. Admin controllers use `namespace Pterodactyl\Http\Controllers\Admin\Extensions\{identifier};` with class `{identifier}ExtensionController`.
